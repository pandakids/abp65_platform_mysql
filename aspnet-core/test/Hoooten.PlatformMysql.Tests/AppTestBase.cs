...
using Hoooten.PlatformMysql.Tests.TestDatas;
...
namespace Hoooten.PlatformMysql.Tests
{
    ...
    /// Provides methods to easily work with <see cref="PlatformMysqlDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<PlatformMysqlTestModule>
    {
        ...
        {
            void NormalizeDbContext(PlatformMysqlDbContext context)
            {
        ...
        protected void UsingDbContext(Action<PlatformMysqlDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PlatformMysqlDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PlatformMysqlDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PlatformMysqlDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PlatformMysqlDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PlatformMysqlDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PlatformMysqlDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PlatformMysqlDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PlatformMysqlDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PlatformMysqlDbContext, Task<T>> func)
        {
            T result;

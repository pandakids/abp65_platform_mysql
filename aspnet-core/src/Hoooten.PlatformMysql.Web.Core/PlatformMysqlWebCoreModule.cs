...
using Hoooten.PlatformMysql.Web.Authentication.JwtBearer;
using Hoooten.PlatformMysql.Web.Authentication.TwoFactor;
using Hoooten.PlatformMysql.Web.Chat.SignalR;
using Hoooten.PlatformMysql.Web.Configuration;

namespace Hoooten.PlatformMysql.Web
{
    [DependsOn(
        typeof(PlatformMysqlApplicationModule),
        typeof(PlatformMysqlEntityFrameworkCoreModule),
        typeof(AbpAspNetZeroCoreWebModule),
        typeof(AbpAspNetCoreSignalRModule),
        typeof(AbpRedisCacheModule), //AbpRedisCacheModule dependency (and Abp.RedisCache nuget package) can be removed if not using Redis cache
        typeof(AbpHangfireAspNetCoreModule) //AbpHangfireModule dependency (and Abp.Hangfire.AspNetCore nuget package) can be removed if not using Hangfire
    )]
    public class PlatformMysqlWebCoreModule : AbpModule
    {
        private readonly IHostingEnvironment _env;
        private readonly IConfigurationRoot _appConfiguration;

        public PlatformMysqlWebCoreModule(IHostingEnvironment env)
        {
            _env = env;
        ...
        public override void PreInitialize()
        {
            ...
            Configuration.Modules.AbpAspNetCore()
                .CreateControllersForAppServices(
                    typeof(PlatformMysqlApplicationModule).GetAssembly()
                );
        ...
        {
            IocManager.RegisterAssemblyByConvention(typeof(PlatformMysqlWebCoreModule).GetAssembly());
        }
        ...
        private void SetAppFolders()
        {
            ...
            {
                var currentAssemblyDirectoryPath = typeof(PlatformMysqlWebCoreModule).GetAssembly().GetDirectoryPathOrNull();
